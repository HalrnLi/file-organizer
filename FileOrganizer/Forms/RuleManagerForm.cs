...
public class RuleManagerForm : Form
{
    ...
    private static bool ShowRuleDialog(string title, ref string name, ref string path, IWin32Window owner)
    {
        var form = new Form
        {
            Text = title,
        ...
        var nameBox = new TextBox { Top = 18, Left = 80, Width = 335, Text = name };

        var pathLabel = new Label { Text = "路径:", Top = 55, Left = 15, Width = 60 };
        var pathBox = new TextBox { Top = 53, Left = 80, Width = 250, Text = path };
        ...
        cancelBtn.Click += (s, e) => form.DialogResult = DialogResult.Cancel;
    ...
    private void AddButton_Click(object? sender, EventArgs e)
    {
        var name = "";
        var path = "";
        if (ShowRuleDialog("添加规则", ref name, ref path, this))
        {
    ...
    private void EditButton_Click(object? sender, EventArgs e)
    {
        ...
        var name = rule.Name;
        var path = rule.Path;
        if (ShowRuleDialog("编辑规则", ref name, ref path, this))
        {
            rule.Name = name;
            rule.Path = path;
            _config.SaveRules();
    ...
    private void ImportButton_Click(object? sender, EventArgs e)
    {
        ...
        if (dlg.ShowDialog() == DialogResult.OK)
        {
            try
            {
                ...
                var imported = System.Text.Json.JsonSerializer.Deserialize<List<Rule>>(json);
    ...
    private void ExportButton_Click(object? sender, EventArgs e)
    {
        ...
        if (dlg.ShowDialog() == DialogResult.OK)
        {
            try
            {
                var json = System.Text.Json.JsonSerializer.Serialize(_config.Rules.ToList(),
